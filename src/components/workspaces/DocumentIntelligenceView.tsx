    ...
    <div className="h-full flex flex-col space-y-4 font-sans text-sm overflow-hidden">
      ...
      <div className="bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-lg p-3 flex flex-wrap items-center justify-between gap-3 select-none flex-shrink-0">
        ...
        <div className="flex items-center gap-3 font-mono text-xs">
          <div className="flex items-center gap-2 bg-[var(--bg-primary)] border border-[var(--border-subtle)] rounded px-3 py-1">
            ...
            <input
              ...
              className="bg-transparent border-none text-xs text-[var(--text-primary)] placeholder-[#666666] focus:outline-none w-40 font-sans"
            />
          </div>
          ...
          <div className="flex items-center gap-1 bg-[var(--bg-primary)] border border-[var(--border-subtle)] rounded p-1">
            <button
              ...
              className="p-1 hover:bg-[var(--border-subtle)] rounded text-[var(--text-primary)] disabled:opacity-30"
            >
              <ChevronLeft className="w-4 h-4" />
            </button>
            <span className="px-2 text-[var(--text-primary)] font-bold min-w-[60px] text-center">
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-4 overflow-hidden">
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between">
                    ...
                    <span className="text-xs font-mono px-1.5 py-0.2 rounded bg-[var(--bg-surface)] text-[#9cdcfe]">
                      {doc.type}
                    </span>
                  </div>
        ...
        <div className="lg:col-span-5 bg-[#181818] border border-[var(--border-subtle)] rounded-lg flex flex-col overflow-hidden shadow-sm">
          <div className="h-8 bg-[var(--bg-surface)] border-b border-[var(--border-subtle)] px-4 flex items-center justify-between font-mono text-xs text-[var(--text-secondary)] flex-shrink-0">
            ...
            <span className="text-[var(--status-healthy)] font-semibold">Local PyMuPDF Parser</span>
          </div>

          <div className="flex-1 overflow-auto p-4 flex items-center justify-center bg-[#141414]">
            <div className="w-[460px] min-h-[500px] bg-[var(--bg-primary)] border border-[var(--border-subtle)] rounded-lg p-6 shadow-2xl font-mono text-xs text-[var(--text-primary)] space-y-4 relative">
              {/* Document Header */}
              <div className="border-b border-[var(--border-subtle)] pb-3 text-center space-y-1">
              ...
              <div className="space-y-3 text-xs leading-relaxed text-[var(--text-primary)]">
                <div className="bg-[var(--bg-surface)] p-3 rounded-md border border-[var(--border-subtle)] space-y-1">
                  <span className="text-[var(--text-secondary)] uppercase font-bold block text-xs">1. SURVEY OBJECTIVE</span>
                ...
                <div className="bg-[var(--bg-surface)] p-3 rounded-md border border-[var(--border-subtle)] space-y-1.5">
                  <span className="text-[var(--text-secondary)] uppercase font-bold block text-xs">2. RECORDED MEASUREMENTS</span>
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    <div>NOMINAL THICKNESS: <span className="text-[var(--text-primary)] font-bold block">5.00 mm</span></div>
                    <div>MEASURED THICKNESS: <span className="text-[#cca700] font-bold block">3.80 mm (Critical)</span></div>
                    <div>PREVIOUS (2022): <span className="text-[var(--text-primary)] block">5.00 mm</span></div>
                    <div>SERVICE TIME: <span className="text-[var(--text-primary)] block">3.50 Years</span></div>
                  </div>
                </div>
              ...
              {showOcrText && (
                <div className="absolute bottom-3 right-3 px-2 py-1 rounded bg-[#1f3a2b] text-[var(--status-healthy)] border border-[#2e5d44] text-xs font-bold">
                  OCR Confidence: 98.9%
                </div>
              )}
            </div>
        ...
        <div className="lg:col-span-4 bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-lg p-4 flex flex-col space-y-3 overflow-y-auto font-mono text-xs shadow-sm">
          <div className="flex items-center justify-between border-b border-[var(--border-subtle)] pb-2 text-sm uppercase font-bold text-[var(--text-primary)]">
            ...
            <span className="text-xs text-[var(--status-healthy)]">PaddleOCR</span>
          </div>

          <div className="space-y-1.5">
            {selectedDoc.extractedEntities.map((ent, idx) => (
              <div key={idx} className="p-2 bg-[var(--bg-primary)] border border-[var(--border-subtle)] rounded flex items-center justify-between text-xs">
                <div>
                  <span className="text-[var(--text-secondary)] text-[10px] block">{ent.label}</span>
                  <span className="text-[var(--text-primary)] font-semibold">{ent.value}</span>
                </div>
                <span className="text-[var(--status-healthy)] font-bold">{ent.confidence}</span>
              </div>
            ))}
          </div>
                ...
                >
                  <div className="flex items-center justify-between font-bold">
                    ...
                    <span className={`px-2 py-0.5 rounded text-xs font-bold ${
                      f.status === 'Warning' ? 'bg-[#332a00] text-[#cca700]' : 'bg-[#1f3a2b] text-[var(--status-healthy)]'
                    }`}>
                      {f.status}
                    </span>
                  </div>
                  <div className="text-xs text-[var(--text-primary)] mt-1 font-sans">
                    Measured: <span className="text-[var(--text-primary)] font-bold">{f.metric}</span> • Threshold: {f.threshold}
                  </div>
                </div>
              ))}
            </div>
