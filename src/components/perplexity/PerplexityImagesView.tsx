...
export const PerplexityImagesView: React.FC = () => {
  ...
  const [selectedImg, setSelectedImg] = useState<{ title: string; src: string; desc: string; tag: string } | null>(null);

  // Extract visual artifacts from messages
  const messageImages = messages.flatMap(m => (m.artifacts || []))
    .filter(a => a.file_type === 'png' || (typeof a.file_name === 'string' && (a.file_name.endsWith('.png') || a.file_name.endsWith('.svg') || a.file_name.endsWith('.jpg'))))
    .map(a => ({
      title: a.file_name,
      src: a.file_path,
      desc: a.description || 'Visual diagram generated by autonomous agent.',
      tag: 'Artifact'
    }));
  ...
  const images = messageImages.length > 0 ? messageImages : defaultLocalImages;
