                ...
                return (
                  <div className="max-w-3xl mx-auto w-full space-y-4">
                    ...
                    <div className="bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-xl p-4 flex items-center justify-between shadow-sm">
                      ...
                      <a
                        href={latestArtifact.downloadUrl || `/static/artifacts/${latestArtifact.name}`}
                        download={latestArtifact.name}
                  ...
                  <div className="max-w-3xl mx-auto w-full bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-xl p-6 shadow-sm space-y-4 font-sans text-xs">
                    <div className="border-b border-[var(--border-subtle)] pb-3 flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="text-sm font-bold text-[var(--text-primary)]">{latestArtifact.name}</div>
                          <div className="text-[10px] font-mono text-[var(--text-secondary)]">Calculated Excel Workbook Deliverable · Grounded by Qwen</div>
                        </div>
                      </div>
                      <a
                        href={latestArtifact.downloadUrl || `/static/artifacts/${latestArtifact.name}`}
                    ...
                    <div className="p-4 bg-[var(--bg-elevated)] border border-[var(--border-subtle)] rounded-lg font-mono text-xs text-[var(--text-primary)]">
                      <div>Workbook Metadata &amp; Calculations:</div>
                      <div className="mt-1 text-[var(--text-secondary)]">{latestArtifact.description}</div>
                      <div className="mt-2 text-[var(--accent-success)] font-bold">✓ Structured by Qwen &amp; validated in deterministic math sandbox</div>
                    </div>
                  </div>
                );
              }

              if (latestArtifact?.type === 'py') {
                return (
                  <div className="max-w-3xl mx-auto w-full bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-xl p-6 shadow-sm space-y-4 font-sans text-xs">
                    <div className="border-b border-[var(--border-subtle)] pb-3 flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-lg bg-blue-950/40 border border-blue-700/40 flex items-center justify-center text-blue-400 font-bold font-mono">
                          PY
                        </div>
                        <div>
                          <div className="text-sm font-bold text-[var(--text-primary)]">{latestArtifact.name}</div>
                          <div className="text-[10px] font-mono text-[var(--text-secondary)]">Executable Python Script Deliverable · Qwen2.5-Coder-7B</div>
                        </div>
                      </div>
                      <a
                        href={latestArtifact.downloadUrl || `/static/artifacts/${latestArtifact.name}`}
                        download={latestArtifact.name}
                        className="px-3.5 py-2 rounded-lg bg-blue-500 text-black font-bold font-mono text-xs flex items-center gap-1.5 hover:bg-blue-400 transition-colors shadow-sm"
                      >
                        <Download className="w-4 h-4" />
                        <span>Download Python Script (.py)</span>
                      </a>
                    </div>
                    <div className="p-4 bg-[var(--bg-elevated)] border border-[var(--border-subtle)] rounded-lg font-mono text-xs text-[var(--text-primary)] space-y-2">
                      <div>Script Description:</div>
                      <div className="text-[var(--text-secondary)]">{latestArtifact.description}</div>
                      <div className="text-[var(--accent-success)] font-bold">✓ Generated by local Qwen2.5-Coder for sandboxed calculation</div>
                    </div>
                  </div>
                );
              }
              ...
              return (
                <div className="max-w-2xl mx-auto w-full bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded p-6 shadow-sm space-y-4 text-xs">
                  ...
                  <div className="space-y-3 text-[var(--text-primary)] leading-relaxed font-sans text-xs">
                    ...
                    <p>
                      <strong>2. Core Findings &amp; Analysis:</strong> All task inputs have been parsed, structured, and validated locally by Qwen under zero external network egress.
                    </p>
                    <p>
                  ...
                  <div className="flex justify-end gap-2 pt-2">
                    ...
                    <a
                      href={latestArtifact?.downloadUrl || `/static/artifacts/${docName}`}
                      download={docName}
